...
<div class="nav">
<ul>
	<li class="employee"><a href="SAadmin.php">Profile</a>
...
<?php
	...
	$sql= "SELECT emp_id, emp_fullname, emp_type FROM employees ORDER BY `emp_fullname`";
	
	echo " <table bgcolor='#8e1106' border= 1 width='95%'>
			<tr bgcolor='#FFFFFF'>
			<th>Employee ID</th>
            <th>Full Name</th>
			<th>Type</th>
		...
		$sql = mysqli_query ($dbc, "SELECT emp_type FROM employees where emp_id = '$empid' ");
		$check = mysqli_fetch_array($sql);
			
			if (isset($_POST['update']))
			{
				//$emp_type = array_keys($_POST['type']);
				$emp_id = $_POST['empid'];
				$emp_type = $_POST['type'];
				
				$update = "UPDATE employees SET emp_type='$emp_type' WHERE emp_id='$empid'";
				
			$update_Result = mysqli_query($dbc, $update);
			}
?>
            <tr><form method="post" action="SAadminDisplay.php?id=<?php echo $row["emp_id"] ?>">
			<td><?php echo $empid;?> <input type="hidden" name="empid" value="<?php $emp_id?>"/> </td>
			<td><?php echo $fullname; ?> <input type="hidden" name="fullname" value="<?php $fullname?>" /> </td>
			<td>			
			<input type="radio" name="type" value="Admin" <?php if ($emp_type=='Admin') { echo "checked"; } ?> >Admin
			<input type="radio" name="type" value="Employee" <?php if ($emp_type=='Employee') { echo "checked"; } ?> >Employee
			<td>
			<input type="submit" name="update[<?=$empid;?>]" value="Update<?php $row["emp_id"]?>">
			</td>
			</form>
			<td>
			<form method="post" action="employeeAdminView.php?id=<?php echo $row["emp_id"];?>">
			<input type="submit" value="View" id="view_btn">
			</form>
			</td>
			</tr>
